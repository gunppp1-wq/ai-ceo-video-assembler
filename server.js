...
app.post("/analyze-extract", async (req, res) => {
  ...
  try {
    ...
    await runCommand(`ffmpeg -y -i "${videoPath}" -vn -acodec libmp3lame -q:a 4 "${audioPath}"`, 300000);

    console.log(`[${jobId}] Step 3: Extracting frames (1 every 2 seconds)...`);
    fs.mkdirSync(frameDir, { recursive: true });
    await runCommand(`ffmpeg -y -i "${videoPath}" -vf "fps=0.5" "${path.join(frameDir, "frame%d.jpg")}"`, 300000);
