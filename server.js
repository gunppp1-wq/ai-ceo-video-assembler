  ...
  try {
    ...
    for (let sceneIdx = 0; sceneIdx < sceneDirs.length; sceneIdx++) {
      ...
      if (realVideoUrl) {
        ...
        const cmd = `ffmpeg -y -threads 1 -i "${rawClipPath}" -t ${durationPerScene} -vf "fps=25,scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920" -an -c:v libx264 -preset ultrafast -x264opts rc-lookahead=5:bframes=0:ref=1 -pix_fmt yuv420p "${clipPath}"`;
        try {
      ...
      const cmd = `ffmpeg -y -threads 1 -framerate ${sourceFps} -i "${sceneDirs[sceneIdx]}/frame%d.jpg" -t ${durationPerScene} -vf "fps=25,scale=1080:1920" -c:v libx264 -preset ultrafast -x264opts rc-lookahead=5:bframes=0:ref=1 -pix_fmt yuv420p "${clipPath}"`;
      await runCommand(cmd, 60000);
      sceneClipPaths.push(clipPath);
    ...
    const finalCmd = `cd ${tmpDir} && ffmpeg -y -threads 1 -i "${concatenatedPath}" -i "${audioPath}" -map 0:v -map 1:a -c:v libx264 -preset ultrafast -x264opts rc-lookahead=5:bframes=0:ref=1 -c:a aac -b:a 96k -shortest -t ${Math.min(audioDuration, 60)} "${outputPath}"`;
